...
namespace engine1 {
  ...
  uint32_t crc32(std::vector<uint8_t> const& bytes) {
    // "inspired" by https://web.archive.org/web/20080303102530/http://c.snippets.org/snip_lister.php?fname=crc_32.c

    static const uint32_t precomputeTable[] = { /* 0xedb88320 */
    ...
    for (uint8_t byte : bytes)
      crc = precomputeTable[((crc) ^ byte) & 0xFF] ^ (crc >> 8);
  ...
  void NetworkClient::Start(void) {
    ...
    if (socket.BindPort(config.port) == false) {
      config.log << "Could not bind to port " << config.port << "\n";
      return;
    }

    valid = true;

    config.log << "Sending hello, world!\n";
    ...
    if (socket.Send(config.hostAddress, config.port, test) == false) {
      config.log << "Could not send to host " << config.hostAddress << "\n";
    }

    recv.reserve(strlen("hello, world\n") + 1);
